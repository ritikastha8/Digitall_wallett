...
import 'package:digital_wallett_system/features/supportmessage/presentation/view_model/support_message_view_model.dart';
...
class SupportMessagesPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SupportMessagesPage> createState() => _SupportMessagesPageState();
}
...
class _SupportMessagesPageState extends ConsumerState<SupportMessagesPage> {
  ...
  Future<void> _saveMessage() async {
    ...
    final error = _editingMessageId == null
        ...
        : await notifier.updateMessage(id: _editingMessageId!, message: message);
  ...
  Future<void> _deleteMessage(String id) async {
    ...
    final error =
        await ref.read(supportMessageViewModelProvider.notifier).deleteMessage(id);
    if (!mounted) return;
    if (error != null) {
      SnackbarUtils.showError(context, error);
      return;
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<SupportMessageState>(supportMessageViewModelProvider, (prev, next) {
      if (!mounted) return;
